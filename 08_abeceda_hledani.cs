    ...
    class Density
    {
        ...
        public void CalculatePercentages(string lorem)
        {
            ...
            foreach (char c in lorem)
            {
                ...
                {
                    secLength++;
                    if (secondary.ContainsKey(prev))
                        secondary[prev]++;
                    else
                        secondary.Add(prev, 1);
                }
                if (prev == 'e')
                {
                    secLength++;
                    if (secondary.ContainsKey(c))
                        secondary[c]++;
                    else
                        secondary.Add(c, 1);
                }
            ...
            foreach (KeyValuePair<int, int> pair in secondary.OrderByDescending(pair => pair.Value))
                Console.WriteLine("{0}: {1} = {2}%", (char)pair.Key, pair.Value, (double)pair.Value / secLength * 100);
        }
